...
describe("SX Launchpad Ecosystem", function () {
  let adminA, adminB, adminC, user1, user2, treasury;
  let usdc, usdt, dai, projectToken;
  let governance, sxp, sxcp, sxua, launchpad, sxep;
  ...
  beforeEach(async function () {
    [adminA, adminB, adminC, user1, user2, treasury] = await ethers.getSigners();
  ...
  describe("Component 2: Launchpad Payment & Forfeiture", function () {
    ...
    beforeEach(async function () {
      ...
      const addProjectData = launchpad.interface.encodeFunctionData("addProject", [
        ...
        buybackStart,
        buybackEnd,
        pricePerToken // Buyback at price
      ]);
    ...
    it("V2.1: Stablecoin payment via SXUA", async function () {
      ...
      expect(await sxua.uncommittedBalances(user1.address, dai.target)).to.equal(depositAmount - expectedCost);
    });
    ...
    it("V2.3: Forfeiture enforcement on early exit", async function () {
      ...
      // User gets 90 LPT, 10 LPT goes to treasury
      expect(await projectToken.balanceOf(user1.address)).to.equal(ethers.parseUnits("90", 18));
      expect(await projectToken.balanceOf(treasury.address)).to.equal(ethers.parseUnits("10", 18));
    });
  });
  ...
  describe("SXUA — Balance Split & Penalty Calculation", function () {
    ...
    describe("C — Early-exit penalty on uncommit", function () {
      ...
      it("C3: day 45 early exit forfeits accrued yield and applies 10% principal penalty", async function () {
        await depositDAI(user1, 1000);
        await sxua.connect(user1).commit(dai.target, D18(1000));
        ...
        const expectedForfeitedYield = D18(54);
        expect(await sxua.accruedRewards(user1.address, dai.target)).to.equal(expectedForfeitedYield);
      ...
      it("C4: day 101 exit has no penalty and returns principal plus accrued yield", async function () {
        ...
        expect(await sxua.accruedRewards(user1.address, dai.target)).to.equal(expectedYield);
        ...
        await sxua.connect(user1).claimDailyYield(dai.target);
        expect(await sxua.uncommittedBalances(user1.address, dai.target)).to.equal(D18(1121));
      });
    ...
    describe("D — Committed balance earns 0.12% daily yield after 24h", function () {
      ...
      it("D8: multiple committed balances created on different dates accrue independently", async function () {
        ...
        const firstBalanceExpectedYield = D18(54);
        expect(await sxua.accruedRewards(user1.address, dai.target)).to.equal(firstBalanceExpectedYield);
        ...
        const secondPeriodExpectedYield = D18(100);
        expect(await sxua.accruedRewards(user1.address, dai.target)).to.equal(
          firstBalanceExpectedYield + secondPeriodExpectedYield
        );

        await sxua.connect(user1).claimDailyYield(dai.target);
        expect(await sxua.uncommittedBalances(user1.address, dai.target)).to.equal(D18(154));
      });
    });
